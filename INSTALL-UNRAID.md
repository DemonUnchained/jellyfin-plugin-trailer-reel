...
| --- | --- | --- |
| Plugin | `/mnt/user/appdata/jellyfin/config/plugins/TrailerReel_0.2.1.4` | `/config/plugins/TrailerReel_0.2.1.4` |
...
Trailer Reel 0.2.1.4 is for Jellyfin 12.1.
...
4. Extract Trailer Reel 0.2.1.4 as described below, recreate only the Jellyfin service, and let startup finish without interruption.
5. Confirm the Dashboard reports Jellyfin 12.1 and Trailer Reel 0.2.1.4 before running the controlled plugin test.
...
```text
/mnt/user/appdata/jellyfin/config/plugins/TrailerReel_0.2.1.4/Jellyfin.Plugin.TrailerReel.dll
```

Do not create a second nested `TrailerReel_0.2.1.4/TrailerReel_0.2.1.4` directory. Keep older Trailer Reel assemblies outside the active `plugins` directory. The unchanged plugin GUID allows the Jellyfin 12.1 build to reuse the existing Trailer Reel settings.
...
```bash
bash /mnt/user/appdata/jellyfin/config/plugins/TrailerReel_0.2.1.4/tools/install-unraid-tools.sh
```
...
There is no new companion container and no change to Jellyfin's network or VPN routing. The only image update required for this compatibility release is the Jellyfin server image itself, using the official `12.1` tag.
...
```bash
bash /mnt/user/appdata/jellyfin/config/plugins/TrailerReel_0.2.1.4/tools/verify-container-tools.sh jellyfin
```
...
For the first controlled test, set **Maximum trailer files** to `3` or `5`. Run **Dashboard → Scheduled Tasks → Download/refresh Trailer Reel** and test a movie from the beginning. Raise the cap to `100` only after the download and preroll behavior are confirmed.
