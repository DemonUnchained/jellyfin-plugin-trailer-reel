...
## Rocinante Applications-pool layout
...
| --- | --- | --- |
| `/mnt/user/appdata/jellyfin/trailer-reel/trailers` | `/trailers` | Read/write |

This location is under Rocinante's applications-backed `appdata` share while keeping the 10–30 GB trailer catalog out of normal Jellyfin configuration backups.

The existing `/mnt/user/appdata/jellyfin/config` → `/config` mapping holds the much smaller downloader tools. The included `tools/install-unraid-tools.sh` downloads the official yt-dlp and Deno Linux releases, verifies their published SHA-256 checksums, and installs them as:
...
If fewer than three indexed matching trailers exist, the plugin returns only the matches it has. It does not fill the queue with unrelated genres.
